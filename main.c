...
int detalhe=0;     // nível de detalhe inicial
...
int main(int argc, char** argv)
{
    ...
    int chan;
    pic.img = SOIL_load_image(argv[1], &pic.width, &pic.height, &chan, SOIL_LOAD_AUTO); //, SOIL_FLAG_INVERT_Y); // | SOIL_FLAG_POWER_OF_TWO);
    if(!pic.img)
    {
    ...
    tex = SOIL_create_OGL_texture(pic.img, pic.width, pic.height, SOIL_LOAD_RGB, SOIL_CREATE_NEW_ID, 0);
